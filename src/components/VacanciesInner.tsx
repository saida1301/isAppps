...
console.log(response.data)
  ...
  if (isLoading) {
    ...
    const isBeforeDeadline = moment(new Date()).isSameOrBefore(data[0]?.deadline, 'day');
    console.log(moment(new Date()).format('DD.MM.YYYY') >= moment(data[0]?.deadline).format('DD.MM.YYYY'));
...
/>
      ...
      <View style={[styles.descriptionContainer, {    backgroundColor: isDarkMode ? '#1b1523' : '#E2DCEA',}]}>
        <Text
          style={[
    ...
    <View style={styles.container2}>
      ...
      <View style={[styles.requirementsContainer, {    backgroundColor: isDarkMode ? '#1b1523' : '#E2DCEA',}]}>
        <Text
          style={[
    ...
    {(data[0]?.accept_type === 1 || data[0]?.accept_type === 3) && isBeforeDeadline && (
            <View style={{ flexDirection: 'row', justifyContent: 'space-around', width: 400, marginTop: 40, marginBottom: 90 }}>
              <Pressable
                onPress={handleIndiMüraciət}
                style={showIndiComponent ? activeButtonStyle : inactiveButtonStyle}
              >
                <Text style={showIndiComponent ? activeButtonTextStyle : inactiveButtonTextStyle}>
                  {t('apply_now')}
                </Text>
              </Pressable>
              <Pressable
                onPress={handleAsanMüraciət}
                style={showAsanComponent ? activeButtonStyle : inactiveButtonStyle}
              >
                <Text style={showAsanComponent ? activeButtonTextStyle : inactiveButtonTextStyle}>
                  {t('easy_apply')}
                </Text>
              </Pressable>
            </View>
          )}
    {data[0]?.accept_type === 0 && (
            <Pressable onPress={handleApply} style={styles.button}>
              <Text style={{ textAlign: "center", margin: 6, color: "#FFFFFF", fontSize: 16, fontWeight: "500" }}>
                {t('apply')}
              </Text>
            </Pressable>
          )}
...
{showIndiComponent && (
  // Render content for Indi Müraciət
<View style={styles.contentContainer}>
  <Picker
    selectedValue={cvId}
    onValueChange={(value) => setCvId(value)}
    style={styles.picker}
  >
    <Picker.Item label="Select a CV" value="" />
    {cvList.map((cv) => (
      <Picker.Item key={cv.id} label={cv.cv} value={cv.id} />
    ))}
  </Picker>

  <Pressable onPress={handleUpload} style={styles.buttonSub}>
    <Text style={{ textAlign: 'center', margin: 6, color: '#FFFFFF', fontSize: 16, fontWeight: '500' }}>
      {t('submit')}
    </Text>
  </Pressable>
